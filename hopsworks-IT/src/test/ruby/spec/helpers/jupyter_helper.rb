...
module JupyterHelper
  ...
  def create_notebook(jupyter_port)
    json_result = post "/bigdata-api/jupyter/#{jupyter_port}/api/contents", {type: "notebook", path: ""}
    expect_status(201)
  ...
  def update_notebook(jupyter_port, content, notebook_name)
    put "/bigdata-api/jupyter/#{jupyter_port}/api/contents/#{notebook_name}", {content: content, format:"json", path:
  ...
  def create_websocket_connection_to_jupyter_server(port, kernel_id, session_id)
    ws = WebSocket::Client::Simple.connect "wss://localhost:8181/bigdata-api/jupyter/#{port}/api/kernels/#{kernel_id}/channels?session_id=#{session_id}"
    return ws
  end

  def create_notebook_session(jupyter_port, notebook_name)
    json_result = post "/bigdata-api/jupyter/#{jupyter_port}/api/sessions", {path:SecureRandom.uuid, name:notebook_name,
                                                                       type:"notebook"}
