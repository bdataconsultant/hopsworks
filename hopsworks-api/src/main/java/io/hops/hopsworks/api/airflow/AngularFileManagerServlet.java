...
//  private String DATE_FORMAT = "EEE, d MMM yyyy HH:mm:ss z"; // (Wed, 4 Jul 2001 12:08:56)
    ...
    try {
      ...
      if (ServletFileUpload.isMultipartContent(request)) {
        if (isSupportFeature(Mode.upload)) {
          ...
          setError(new IllegalAccessError((String) notSupportFeature(Mode.upload).get("error")), response);
        }
  ...
  private void uploadFile(HttpServletRequest request, HttpServletResponse response) throws ServletException {
    ...
    if (isSupportFeature(Mode.upload)) {
      ...
      throw new ServletException((String) notSupportFeature(Mode.upload).get("error"));
    }
  }
...
//            JSONObject params = JSONValue.parse(sb.toString(), JSONObject.class);
  ...
  private JSONObject move(JSONObject params) throws ServletException {
    try {
      ...
      Path newpath = Paths.get(REPOSITORY_BASE_PATH, (String) params.get("newPath"));
...
//      for (Object obj : paths) {
  ...
  private JSONObject copy(JSONObject params) throws ServletException {
    try {
      JSONArray paths = ((JSONArray) params.get("items"));
      Path newpath = Paths.get(REPOSITORY_BASE_PATH, (String) params.get("newPath"));
      String newFileName = (String) params.get("singleFilename");
//            for (Object obj : paths) {
      for (int i = 0; i < paths.length(); i++) {
        JSONObject obj = paths.getJSONObject(i);
...
//      Path path = Paths.get(REPOSITORY_BASE_PATH, obj.toString());
  ...
  private JSONObject changePermissions(JSONObject params) throws ServletException {
    try {
      JSONArray paths = ((JSONArray) params.get("items"));
      String perms = (String) params.get("perms"); // "rw-r-x-wx"
      String permsCode = (String) params.get("permsCode"); // "653"
      boolean recursive = "true".equalsIgnoreCase((String) params.get("recursive"));
//      for (Object path : paths) {
  ...
  private JSONObject compress(JSONObject params) throws ServletException {
    try {
      JSONArray paths = ((JSONArray) params.get("items"));
      String paramDest = (String) params.get("destination");
      final Path dest = Paths.get(REPOSITORY_BASE_PATH, paramDest);
      Path zip = dest.resolve((String) params.get("compressedFilename"));
      if (Files.exists(zip)) {
...
//        for (Object path : paths) {
  ...
  private JSONObject extract(JSONObject params) throws ServletException {
    ...
    Path dest = Paths.get(REPOSITORY_BASE_PATH, (String) params.get("destination"));
    final Path folder = dest.resolve((String) params.get("folderName"));
    try {
      if (!Files.isDirectory(folder)) {
        genFolder = true;
        Files.createDirectories(folder);
      }
      String zip = (String) params.get("item");
      Map<String, String> env = new HashMap<>();
      env.put("create", "false");
      try (FileSystem zipfs = FileSystems.newFileSystem(URI.create("jar:file:" + Paths.get(REPOSITORY_BASE_PATH, zip).
          toString()), env)) {
        Files.walkFileTree(zipfs.getPath("/"), new SimpleFileVisitor<Path>() {
          ...
          @Override
          public FileVisitResult preVisitDirectory(Path dir, BasicFileAttributes attrs) throws IOException {
            Path subFolder = folder.resolve(dir.getNameCount() < 1 ? "" : 
                dir.subpath(0, dir.getNameCount()).toString());
